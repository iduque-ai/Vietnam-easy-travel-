...
import { Download, Check, MapPin, Star, Clock, Ticket, AlertTriangle, Navigation, ExternalLink, Bookmark, BookmarkCheck, FileText, Share2, Compass, Eye, Filter, CalendarPlus, X, Sparkles } from 'lucide-react';
...
export const DownloadableMaps: React.FC<DownloadableMapsProps> = ({ ratesData, isOnline, onNavigateToItinerary }) => {
    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 space-y-4">
          {/* Interactive SVG Vector Map of Vietnam */}
          <div className="bg-stone-900 text-stone-100 rounded-2xl p-5 border border-stone-800 shadow-md relative overflow-hidden">
            <div className="flex items-center justify-between mb-3">
              <div>
                <span className="text-[11px] text-amber-400 uppercase font-semibold tracking-wider">
              ...
              <span className="text-xs bg-stone-800 text-stone-300 px-2.5 py-1 rounded-md border border-stone-700">
                {regionPois.length} ubicaciones
              </span>
            </div>

            {/* Interactive SVG Vietnam Map Representation */}
            <div className="relative w-full h-80 bg-stone-950/80 rounded-xl border border-stone-800 p-2 overflow-hidden flex items-center justify-center">
              <svg viewBox="0 0 500 400" className="w-full h-full">
                ...
                {regionPois.map((poi, idx) => {
                  // Project lat/lng roughly to SVG coordinate space
                  // Vietnam Lat ~ 9 to 23, Lng ~ 102 to 109
                  // Let's position pins nicely within the region
              ...
              <div className="absolute bottom-2 left-2 bg-stone-900/90 backdrop-blur-xs px-2.5 py-1 rounded-md text-[10px] text-stone-400 border border-stone-800">
                📍 Toca un pin para ver detalles, tarifas y estafas a evitar
              </div>
            </div>
        ...
        <div className="lg:col-span-5">
          {selectedPoi ? (
            <div className="bg-white rounded-2xl p-5 sm:p-6 border border-stone-200 shadow-sm space-y-4 sticky top-24">
              ...
              {/* Actions: Add to Itinerary and Google Maps */}
              <div className="pt-2 space-y-2">
                ...
                <a
                  ...
                  className="w-full py-2 px-3 rounded-xl bg-stone-900 hover:bg-stone-800 text-white font-semibold text-xs flex items-center justify-center gap-1.5 transition"
                >
                  <ExternalLink className="w-3.5 h-3.5" />
                  <span>Abrir en Google Maps / Maps.me ({selectedPoi.lat}, {selectedPoi.lng})</span>
                </a>
              </div>
            </div>
          ) : (
